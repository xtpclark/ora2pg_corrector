...
rem ACCEPT pass PROMPT 'Enter a password for the user HR: ' HIDE
...
rem ACCEPT tbs PROMPT 'Enter a tablespace for HR [&var_default_tablespace]: ' DEFAULT '&var_default_tablespace'

DEFINE var_default_tablespace=USERS
...
rem DEFINE overwrite_schema=YES
...
rem       FROM all_users WHERE username = 'HR';
...
rem         EXECUTE IMMEDIATE 'DROP USER HR CASCADE';
rem         DBMS_OUTPUT.PUT_LINE('Old HR schema has been dropped.');
...
rem CREATE USER hr IDENTIFIED BY "&pass"
rem                DEFAULT TABLESPACE &tbs
 rem               QUOTA UNLIMITED ON &tbs;
